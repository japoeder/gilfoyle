...
def ticker_expansion():
    ...
    try:
        with open(f'{os.getenv("JOB_CTRLS")}/mlops.json', 'r') as file:
    ...
    for ticker in tickers:
        ...
        days = [datetime.strptime(doc['_id'], "%Y-%m-%d").date() for doc in raw_collection.aggregate(pipeline)]
...
def get_last_close(collection, ticker, current_time):
    ...
    previous_data = collection.find_one(
        {"ticker": ticker, "timestamp": {"$lt": current_time}},
        sort=[("timestamp", -1)]
    )
